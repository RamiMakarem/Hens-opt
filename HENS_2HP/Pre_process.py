...
def pre_process_milp(Hsap, Csap, delta_tmin, qh, qc,
    ...
    from A_Beta_HP import generate_abeta_hyperplanes_for_match

    # ════════════════════════════════════════════════════════════════════
    # Dynamic Hyperplane Preprocessing for A^beta (Heat Exchanger Area)
    # ════════════════════════════════════════════════════════════════════

    # Primary dictionary indexed by (i, j, p) for Pyomo constraint generation
    hp_data = {}             

    # Supplementary dictionary structures for reference / debugging
    hyperplanes_abeta = {}   # (i, j) -> list of (a0, a1, a2)
    grid_Q_match = {}        # (i, j) -> 1D numpy array of Q breakpoints
    grid_LMTD_match = {}     # (i, j) -> 1D numpy array of LMTD breakpoints
    match_plane_keys = {}    # (i, j) -> list of plane indices range(n_planes)

    GRID_MODE = "dynamic"    # "fixed" or "dynamic"
    ERR_TARGET = 0.01        # 1% max relative error target if mode="dynamic"
    ...
    for i in Hi:
        for j in Hj:
            ...
            # Domain bounds for Q and LMTD for match (i, j)
            Q_lo_ij = max(0.01 * Q_match_max[i, j], EPSILON)
            Q_hi_ij = Q_match_max[i, j]

            L_lo_ij = _lmtd(dT1_lo[i, j], dT2_lo[i, j])
            L_hi_ij = _lmtd(dT1_hi[i, j], dT2_hi[i, j])

            # Ensure valid non-degenerate intervals
            if Q_hi_ij <= Q_lo_ij or L_hi_ij <= L_lo_ij:
                continue

            # Generate match-specific tangent hyperplanes
            planes_ij, g_Q_ij, g_L_ij = generate_abeta_hyperplanes_for_match(
                Q_lo=Q_lo_ij, Q_hi=Q_hi_ij,
                L_lo=L_lo_ij, L_hi=L_hi_ij,
                U=U_ij, beta=cost_beta,
                N_G=N_G_process,
                mode=GRID_MODE,
                error_threshold=ERR_TARGET
            )

            n_q, n_l = len(g_Q_ij), len(g_L_ij)

            # 1. Calculate midpoint cell boundaries for Q and LMTD grid domains
            q_cells = [
                (Q_lo_ij if idx == 0 else 0.5 * (g_Q_ij[idx - 1] + g_Q_ij[idx]),
                Q_hi_ij if idx == n_q - 1 else 0.5 * (g_Q_ij[idx] + g_Q_ij[idx + 1]))
                for idx in range(n_q)
            ]

            l_cells = [
                (L_lo_ij if idx == 0 else 0.5 * (g_L_ij[idx - 1] + g_L_ij[idx]),
                L_hi_ij if idx == n_l - 1 else 0.5 * (g_L_ij[idx] + g_L_ij[idx + 1]))
                for idx in range(n_l)
            ]

            # 2. Populate unified hp_data dictionary with plane equations and bounds
            p = 0
            for idx_q in range(n_q):
                for idx_l in range(n_l):
                    a0, a1, a2 = planes_ij[p]
                    hp_data[i, j, p] = {
                        "a0": float(a0),
                        "a1": float(a1),
                        "a2": float(a2),
                        "q_min": float(q_cells[idx_q][0]),
                        "q_max": float(q_cells[idx_q][1]),
                        "l_min": float(l_cells[idx_l][0]),
                        "l_max": float(l_cells[idx_l][1]),
                    }
                    p += 1

            # 3. Store raw lookup structures
            hyperplanes_abeta[i, j] = planes_ij
            grid_Q_match[i, j] = g_Q_ij
            grid_LMTD_match[i, j] = g_L_ij
            match_plane_keys[i, j] = list(range(len(planes_ij)))
...
def build_model(data):
    ...
    # ---- A^beta Hyperplane Sets ----
    # 1. Extract all unique plane indices p present in preprocessed data
    all_planes = set(p for (i, j, p) in getattr(data, "hp_data", {}).keys())
    max_plane_idx = max(all_planes) if all_planes else 0

    # 2. Global plane index range set (0 .. max_plane_idx)
    m.K_planes = pyo.RangeSet(0, max_plane_idx)

    # 3. Sparse (i, j, k, p) tuple set for match-specific valid plane combinations
    match_planes = {}
    for (i, j, p) in getattr(data, "hp_data", {}).keys():
        match_planes.setdefault((i, j), []).append(p)

    valid_ijkp = [
        (i, j, k, p)
        for (i, j, k) in m.FeasibleIJK
        for p in match_planes.get((i, j), [])
    ]
    m.FeasibleIJKP = pyo.Set(initialize=valid_ijkp, dimen=4)
