"""
LMTD Tangent-Plane (1st-order Taylor) Hyperplane Tester
=========================================================
"""
...
def lmtd_true(dT1, dT2, tol=0.1):
    L, _, _ = lmtd_and_grad(dT1, dT2, tol)
    return L


def tangent_plane_coeffs(dT1_0, dT2_0, tol=1e-6):
    """Returns (a0,a1,a2) s.t. LMTD(dT1,dT2) <= a0 + a1*dT1 + a2*dT2."""
    L0, g1, g2 = lmtd_and_grad(dT1_0, dT2_0, tol)
    a0 = L0 - g1 * dT1_0 - g2 * dT2_0
    return a0, g1, g2
...
# ──────────────────────────────────────────────────────────────────────
def build_tangent_hyperplanes(dT_lo, dT_hi, N_G, use_symmetry=True):
    """
    ...
    use_symmetry=True: exploits LMTD(dT1,dT2) == LMTD(dT2,dT1). Only
    the upper-triangular N_G*(N_G+1)/2 points need an actual gradient
    ...
    if use_symmetry:
        for i in range(N_G):
            for j in range(i, N_G):
                x, y = grid[i], grid[j]
                a0, a1, a2 = tangent_plane_coeffs(x, y)
        ...
        for x in grid:
            for y in grid:
                planes.append(tangent_plane_coeffs(x, y))

    return planes, grid


def envelope_value(dT1, dT2, planes):
    """Piecewise-linear outer approximation = min over all tangent planes."""
...
# ──────────────────────────────────────────────────────────────────────
def test_against_dense_grid(planes, dT_lo, dT_hi, dense_N=100, use_geomspace=True):
    """
    Evaluates true LMTD vs. hyperplane envelope over a dense
    dense_N x dense_N test grid and reports error statistics.
    """
    ...
    L_true = lmtd_true(D1, D2)
    L_approx = envelope_value(D1, D2, planes)

    err = L_approx - L_true                       # should be >= ~0 (overestimate)
    rel_err = err / np.maximum(L_true, 1e-9)

    idx = np.unravel_index(np.argmax(rel_err), rel_err.shape)
    n_violations = int(np.sum(err < -1e-6))        # sign-consistency check

    return {
        "grid": g, "D1": D1, "D2": D2, "L_true": L_true, "L_approx": L_approx,
...
# minimum, error(dT1,dT2) = plane(dT1,dT2) - LMTD(dT1,dT2) is a LINEAR
# function minus a CONCAVE one -> CONVEX on that region. A convex
# function's max over a region is always on the region's boundary, not
# the interior. So the true worst-case error lives on the domain edges,
# the dT1==dT2 diagonal, or a plane-switching curve -- a uniform/dense
# grid can straddle these without ever sampling exactly on them.
# ──────────────────────────────────────────────────────────────────────
...
def test_against_structured_points(planes, dT1_pts, dT2_pts):
    """Same error stats as test_against_dense_grid, but for a flat point set."""
    L_true = lmtd_true(dT1_pts, dT2_pts)
    L_approx = envelope_value(dT1_pts, dT2_pts, planes)
    err = L_approx - L_true
    rel_err = err / np.maximum(L_true, 1e-9)
    idx = int(np.argmax(rel_err))
    n_violations = int(np.sum(err < -1e-6))
    return {
        "dT1": dT1_pts, "dT2": dT2_pts, "L_true": L_true, "L_approx": L_approx,
        "err": err, "rel_err": rel_err,
...
def full_validation(planes, dT_lo, dT_hi, dense_N=100, n_per_edge=200, use_geomspace=True):
    """
    ...
    """
    dense_result = test_against_dense_grid(planes, dT_lo, dT_hi, dense_N, use_geomspace)
    dT1_b, dT2_b = build_boundary_and_diagonal_points(dT_lo, dT_hi, n_per_edge, use_geomspace)
    boundary_result = test_against_structured_points(planes, dT1_b, dT2_b)
...
def dense_grid_convergence_check(planes, dT_lo, dT_hi,
    ...
    for dense_N in dense_N_list:
        result = full_validation(planes, dT_lo, dT_hi, dense_N=dense_N, n_per_edge=n_per_edge)
...
# ──────────────────────────────────────────────────────────────────────
def select_grid_size_dynamic(dT_lo, dT_hi, error_threshold=0.02,
    ...
    for N_G in range(N_start, N_max + 1):
        planes, _grid = build_tangent_hyperplanes(dT_lo, dT_hi, N_G, use_symmetry)
        result = full_validation(planes, dT_lo, dT_hi, dense_N=dense_N, n_per_edge=n_per_edge)
...
# 3D plotting
# ──────────────────────────────────────────────────────────────────────
def plot_3d_comparison(result, title="LMTD: true surface vs. tangent-plane envelope",
    ...
    for k, (elev, azim) in enumerate(angles):
        ...
        ax.set_xlabel("dT1"); ax.set_ylabel("dT2"); ax.set_zlabel("LMTD")
...
def plot_error_surface(result, angles=((25, -60), (60, -45))):
    ...
    for k, (elev, azim) in enumerate(angles):
        ...
        ax.set_title(f"elev={elev}, azim={azim}")
        ax.view_init(elev=elev, azim=azim)
    fig.suptitle("Relative approximation error (%) over (dT1, dT2)")
    fig.tight_layout()
    return fig
...
# Top-level driver
# ──────────────────────────────────────────────────────────────────────
def analyze_match(dT_hi, delta_tmin,
    ...
    Validation now combines the dense interior grid with targeted
    ...
    #print(f"Match: Tin_H={Tin_H}, Tin_C={Tin_C}, delta_tmin={delta_tmin}")
    print(f"dT range: [{dT_lo:.4f}, {dT_hi:.4f}]\n")

    if mode == "dynamic":
        print("Running dynamic grid-size sweep...")
        N_G, planes, result, history = select_grid_size_dynamic(
            dT_lo, dT_hi, error_threshold, convergence_tol,
    ...
    if check_dense_N_convergence:
        ...
        dense_grid_convergence_check(planes, dT_lo, dT_hi, dense_N_sweep, n_per_edge)
    ...
    print(f"Sign violations     : {result['n_sign_violations']} "
          f"({'OK - strictly overestimating' if result['n_sign_violations'] == 0 else 'WARNING: non-monotonic direction found!'})")
